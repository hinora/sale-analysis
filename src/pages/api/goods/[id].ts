...
export default async function handler(
  ...
  try {
    ...
    const averagePrice =
      totalQuantityExported > 0
        ? totalValueExported / totalQuantityExported
        : 0;
    ...
    console.log(
      `[API] Returning goods detail for ${id} with ${formattedTransactions.length} transactions`,
    );
    ...
    return res.status(200).json({
      goods: {
        ...
        classifiedAt: goods.classifiedAt
          ? goods.classifiedAt.toISOString()
          : "",
