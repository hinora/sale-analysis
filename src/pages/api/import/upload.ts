...
export default async function handler(req: NextApiRequest, res: NextApiResponse<UploadResponse>) {
  ...
  try {
    ...
    if (!validation.valid) {
      return res.status(400).json({
        ...
        stats: {
          ...
          newCompanies: 0,
          newGoods: 0,
          errors: validation.errors.map((e) => `Row ${e.row}: ${e.field} - ${e.message}`),
        },
      });
    }
    ...
    const uniqueDeclarationNumbers = Array.from(
      new Set(fileDedupeResult.uniqueRows.map((row: CSVRow) => row['Số tờ khai']))
    );
    ...
    const existingCompositeKeys = new Set(
      existingTransactions.map((t) => `${t.declarationNumber}|${t.hsCode}|${t.goodsRawName}`)
    );
    ...
    console.log('[Import] AI disabled for performance - using fallback classification');
    ...
    for (const row of dbDedupeResult.newRows) {
      try {
        ...
        stats.errors.push(`Failed to import row with declaration ${row['Số tờ khai']}`);
      }
    }
 ...
 */
async function processCompany(row: CSVRow): Promise<{ id: string; isNew: boolean }> {
 ...
 */
async function processGoods(row: CSVRow): Promise<{ id: string; isNew: boolean }> {
