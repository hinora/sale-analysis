...
export function checkSynonyms(
  ...
  const allSynonyms = { ...synonymsData.countries, ...synonymsData.companies, ...synonyms };
  ...
  for (const variants of Object.values(allSynonyms)) {
    ...
    const hasValue1 = normalizedVariants.some(v => v === normalizedValue1 || normalizedValue1.includes(v) || v.includes(normalizedValue1));
    const hasValue2 = normalizedVariants.some(v => v === normalizedValue2 || normalizedValue2.includes(v) || v.includes(normalizedValue2));
...
export function matchesFilter(
  ...
  switch (matchStrategy) {
    ...
    case 'fuzzy': {
      ...
      if (normalizedField.includes(normalizedFilter) || normalizedFilter.includes(normalizedField)) {
        return true;
      }
