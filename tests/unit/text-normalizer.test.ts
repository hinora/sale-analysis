...
describe("text-normalizer", () => {
  describe("normalizeText", () => {
    ...
    it("should preserve case when caseSensitive is true", () => {
      expect(normalizeText("Hello World", { caseSensitive: true })).toBe(
        "Hello World",
      );
    });
    ...
    it("should remove diacritics when requested", () => {
      expect(normalizeText("điện tử", { removeDiacritics: true })).toBe(
        "dien tu",
      );
  ...
  describe("matchesFilter - contains matching", () => {
    it("should match case-insensitively", () => {
      expect(
        matchesFilter("United States", "US", { matchStrategy: "contains" }),
      ).toBe(false); // substring not present
      expect(matchesFilter("USA", "us", { matchStrategy: "contains" })).toBe(
        true,
      );
      expect(
        matchesFilter("CÔNG TY ABC", "abc", { matchStrategy: "contains" }),
      ).toBe(true);
    });

    it("should match with whitespace normalization", () => {
      expect(
        matchesFilter("  hello   world  ", "hello world", {
          matchStrategy: "contains",
        }),
      ).toBe(true);
      expect(
        matchesFilter("hello world", "world", { matchStrategy: "contains" }),
      ).toBe(true);
    });

    it("should match Vietnamese text", () => {
      expect(
        matchesFilter("điện tử", "điện", { matchStrategy: "contains" }),
      ).toBe(true);
      expect(
        matchesFilter("Công ty điện tử", "điện tử", {
          matchStrategy: "contains",
        }),
      ).toBe(true);
    });
  ...
  describe("matchesFilter - exact matching", () => {
    it("should match exactly", () => {
      expect(matchesFilter("USA", "usa", { matchStrategy: "exact" })).toBe(
        true,
      );
      expect(matchesFilter("USA", "us", { matchStrategy: "exact" })).toBe(
        false,
      );
    });
  });
  ...
  describe("matchesFilter - startsWith matching", () => {
    it("should match prefix", () => {
      expect(
        matchesFilter("United States", "united", {
          matchStrategy: "startsWith",
        }),
      ).toBe(true);
      expect(
        matchesFilter("United States", "states", {
          matchStrategy: "startsWith",
        }),
      ).toBe(false);
    });
  ...
  describe("matchesFilter - fuzzy matching", () => {
    it("should match with typos within threshold", () => {
      expect(
        matchesFilter("electronic", "electonic", {
          matchStrategy: "fuzzy",
          fuzzyThreshold: 2,
        }),
      ).toBe(true);
    ...
    it("should handle contains fallback for fuzzy", () => {
      expect(
        matchesFilter("electronics company", "elec", {
          matchStrategy: "fuzzy",
          fuzzyThreshold: 1,
        }),
      ).toBe(true);
  ...
  describe("matchesFilter - null/undefined handling", () => {
    ...
    it("should handle numeric values", () => {
      expect(matchesFilter(12345, "123", { matchStrategy: "contains" })).toBe(
        true,
      );
      expect(matchesFilter(12345, 12345, { matchStrategy: "exact" })).toBe(
        true,
      );
    });

    it("should handle boolean values", () => {
      expect(matchesFilter(true, "true", { matchStrategy: "exact" })).toBe(
        true,
      );
      expect(matchesFilter(false, "false", { matchStrategy: "exact" })).toBe(
        true,
      );
    });
  });
